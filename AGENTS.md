...
- **The app header is route-driven.** `web/src/routes/app/route.tsx` is a layout that
  renders a shared `<AppHeader>` and reads the leaf route's `staticData` through
  `useMatches({ select })`. Each page declares what its header needs —
  `staticData: { title, actions, filters, search }`. `actions` and `filters` are string
  ids resolved via the `HEADER_ACTIONS` / `HEADER_FILTERS` registries; add new ids to
  `@/models/header`. `staticData` is serialized for SSR, so it must hold only
  primitives — never JSX or component references.
